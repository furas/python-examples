...
per pixel transparency - convert_alpha() and (R,G,B,A)
"""
...
def create_image(color, R, alpha): # "alpha" never used
    
    # create surface for circle
    image = pygame.surface.Surface((2*R, 2*R)).convert_alpha()
    
    # 100% transparent black background - ALPHA=0 works
    image.fill((0,0,0,0)) 

    # 100% transparent black background - it doesn't work
    #(black is default color for new surface - don't have to use fill((0,0,0)) )
    #image.set_colorkey((0,0,0)) 
    
    # 0%-100% transparent all other colors - it doesn't work
    ...
    # it works
    pygame.draw.circle(image, (*color, 64),  (R, R), R)      # points with ALPHA=20
    pygame.draw.circle(image, (*color, 128), (R, R), 3*R//4) # points with ALPHA=40
    pygame.draw.circle(image, (*color, 192), (R, R), 2*R//4) # points with ALPHA=60 
...
while running:
    ...
    # set new transparency - it doesn't work
