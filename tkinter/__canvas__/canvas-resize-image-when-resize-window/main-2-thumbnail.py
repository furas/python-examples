...
def on_resize(event):
    ...
    new_image = image.copy()   # it modifies original image so it needs `copy()
    new_image.thumbnail(size)  # it keeps  aspect ratio , DOESN'T crop, but can't create bigger than original image
